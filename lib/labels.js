...
const PROCEDIMIENTO_RULES = [
  ...
  {
    canonical: 'Renovación',
    test: (c) => /^renovacion(?:\s|$)/.test(c) || c === 'renovacion',
  },
  {
    canonical: 'Ampliación',
    test: (c) =>
  ...
  {
    canonical: 'Continuidad de servicio',
    test: (c) => /continuidad\s*de\s*servicio/.test(c),
  },
  {
    canonical: 'Prórroga',
...
export function normalizeProcedimientoTipo(raw) {
  ...
  for (const t of TIPOS_PROCEDIMIENTO) {
    if (compact(t) === compact(text)) return t;
  }
...
export function splitProcesoTipoNumero(proceso_tipo_numero) {
  ...
  if (/^\d+$/.test(s)) {
    return { tipo: 'Licitación Privada', numero: s };
  }
