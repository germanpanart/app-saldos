...
const tipoLabel = (t) => (t === 'obras' ? 'Obras Públicas' : 'Educación Infra');
...
export async function getTramites(supabase) {
  ...
  return (tr || []).map((t) => ({
    ...
    secretaria: t.secretaria || '(sin secretaría)',
    item: t.nro_item || '',
...
// Nombres existentes para los datalist del formulario
export async function getLookups(supabase) {
  ...
  return {
    secretarias: (sec.data || []).map((r) => r.nombre),
    proveedores: (prov.data || []).map((r) => r.nombre),
    rubros: (rub.data || []).map((r) => r.nombre),
  };
}

// Trámite + pagos para el formulario de edición (lookups resueltos a nombre)
export async function getTramiteForEdit(supabase, id) {
  ...
  const initial = {
    id: data.id,
    tipo: data.tipo,
    secretaria: data.secretarias?.nombre || '',
