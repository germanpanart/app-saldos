...
export async function getTramites(supabase) {
  ...
  return {
    ...
    procedimientoTipo: procedimientoTipo(t),
    // N° visible: SOLO nro_item. Nunca SP ni otro campo (aunque el ítem esté duplicado).
    procedimientoNro: (t.nro_item || '').trim(),
    secretaria: secretariaDisplay(areaKey, t.secretaria),
    tramiteNro: tramiteNumero(t),
    item: (t.nro_item || '').trim(),
