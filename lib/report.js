...
function tramiteMetaLine(o) {
  return [
    ...
    `N° ítem: ${dash(o.procedimientoNro || o.item)}`,
    `SP: ${dash(o.tramiteNro)}`,
    `OC N°: ${dash(o.ocNum)}`,
  ].join('   |   ');
}
...
export async function exportPDF(ocs, filtrosTxt) {
  ...
  doc.autoTable({
    ...
    head: [[
      'OC N°', 'N° ítem', 'Procedimiento', 'Secretaría', 'Proveedor', 'Descripción',
...
export async function exportDOCX(ocs, filtrosTxt) {
  ...
  body += th(['OC N°', 'N° ítem', 'Procedimiento', 'Secretaría', 'Proveedor', 'Monto OC', 'Pagado', 'Saldo', '% Av.']);
  ocs.forEach((o) => {
    body += tr([
