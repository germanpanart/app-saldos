...
export async function exportPDF(ocs, filtrosTxt) {
  ...
  doc.text('Informe de Saldos - Ordenes de Compra', 40, 40);
  doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(90);
  doc.text(`Direccion de Compras y Contrataciones - Generado: ${hoy()}`, 40, 56);
  ...
  doc.autoTable({
    ...
    head: [['OC N', 'Tipo', 'Secretaria', 'Proveedor', 'Descripcion', 'Monto OC', 'Pagado', 'Saldo', '% Av.']],
    body: ocs.map((o) => [
...
function th(cells) {
  return `<tr>${cells.map((c) => `<th style="border:1px solid #ccc;padding:4px;background:#2563eb;color:#fff;">${cleanText(c)}</th>`).join('')}</tr>`;
}
function tr(cells, alignRightFrom = -1) {
  return `<tr>${cells.map((c, i) => {
    const align = i >= alignRightFrom ? 'right' : 'left';
    return `<td style="border:1px solid #ccc;padding:4px;text-align:${align};">${cleanText(c)}</td>`;
  }).join('')}</tr>`;
}
...
export async function exportDOCX(ocs, filtrosTxt) {
  ...
  body += `<h1>Informe de Saldos - Ordenes de Compra</h1>`;
  body += `<p><i>Direccion de Compras y Contrataciones - Generado: ${cleanText(hoy())}</i></p>`;
  ...
  body += '<table cellspacing="0" cellpadding="0" style="border-collapse:collapse;width:100%;font-size:10pt;">';
  body += th(['OC N', 'Tipo', 'Secretaria', 'Proveedor', 'Monto OC', 'Pagado', 'Saldo', '% Av.']);
  ocs.forEach((o) => {
    body += tr([o.ocNum, o.tipo, o.secretaria, o.proveedor, fmtARS(o.ocMonto), fmtARS(o.totalPagado), fmtARS(o.saldo), fmtPct(o.avance)], 4);
  });
  body += '</table>';

  body += '<h2>Detalle e historial de pagos</h2>';
  ocs.forEach((o) => {
    body += `<h3>${cleanText(o.tipo)} - OC ${cleanText(o.ocNum)} - ${cleanText(o.descripcion)}</h3>`;
    body += `<p><i>${cleanText(o.secretaria)} - ${cleanText(o.proveedor)} - Monto OC: ${cleanText(fmtARS(o.ocMonto))} - Saldo: ${cleanText(fmtARS(o.saldo))}</i></p>`;
    body += '<table cellspacing="0" cellpadding="0" style="border-collapse:collapse;width:100%;font-size:10pt;margin-bottom:16px;">';
...
<style>
  ...
  h1 { font-size: 16pt; } h2 { font-size: 13pt; margin-top: 18px; } h3 { font-size: 11pt; }
</style>
</head>
<body>${body}</body>
</html>`;
